...
# Auto-load .env from project root (no-op if file is missing)
...
def load_spaces_config() -> SpacesConfig:
    ...
    if not access_key:
        raise EnvironmentError(
            "SPACES_ACCESS_KEY_ID is not set. "
            "Set it in your environment or .env file."
        )
    if not secret_key:
        raise EnvironmentError(
            "SPACES_SECRET_ACCESS_KEY is not set. "
            "Set it in your environment or .env file."
        )
    ...
    return SpacesConfig(
        ...
        bucket=os.environ.get("SPACES_BUCKET", "airlines-sim").strip(),
